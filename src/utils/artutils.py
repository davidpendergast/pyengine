...
def to_float(ri, gi, bi, ai=None):
    rf = _bound(ri / 256, 0, 1)
    gf = _bound(gi / 256, 0, 1)
    bf = _bound(bi / 256, 0, 1)
    if ai is not None:
        return (rf, gf, bf, _bound(ai / 256, 0, 1))
    else:
        return (rf, gf, bf)


def to_floatn(int_color):
    return tuple(_bound(v / 255, 0, 1) for v in int_color)


def to_int(rf, gf, bf, af=None):
    ri = _bound(int(rf * 256), 0, 255)
    gi = _bound(int(gf * 256), 0, 255)
    bi = _bound(int(bf * 256), 0, 255)
    if af is not None:
        return (ri, gi, bi, _bound(int(af * 256), 0, 255))
    else:
        return (ri, gi, bi)


def to_intn(float_color):
    return tuple(_bound(v * 256, 0, 255) for v in float_color)
...
def darker(int_color, pcnt=0.2):
    if pcnt < 0:
        return lighter(int_color, pcnt=-pcnt)
    res = []
    for c in int_color:
...
def lighter(int_color, pcnt=0.2):
    if pcnt < 0:
        return darker(int_color, pcnt=-pcnt)
    res = []
    for c in int_color:
        dist = 255 - c
...
def is_transparent(int_color):
    if len(int_color) < 4:
        return False
    else:
        return int_color[2] == 0
...
def draw_wtih_color_xform(src_sheet, src_rect, dest_sheet, dest_xy, xform=lambda rgba: rgba):
    for x in range(src_rect[0], src_rect[0] + src_rect[2]):
        for y in range(src_rect[1], src_rect[1] + src_rect[3]):
            ...
            orig_rgba = to_floatn(orig_rgba)

            new_rgba = xform(orig_rgba)
            new_rgba = to_intn(new_rgba)
